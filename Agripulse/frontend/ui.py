...
def reset_analyse_fields():
    """Réinitialise les champs de l'onglet analyse dans session_state."""
    for key in ['ana_cat', 'ana_produit', 'ana_region', 'ana_prix']:
        if key in st.session_state:
            if key == 'ana_prix':
                st.session_state[key] = 0
            else:
                st.session_state[key] = "— Choisir —"
    st.session_state.region_analyse = None
    st.toast("🔄 Formulaire d'analyse réinitialisé", icon="🔄")

def reset_terrain_fields():
    """Réinitialise les champs de l'onglet terrain dans session_state."""
    for key in ['terrain_cat', 'terrain_produit', 'terrain_region', 'terrain_marche', 'terrain_prix', 'terrain_comm']:
        if key in st.session_state:
            if key == 'terrain_prix':
                st.session_state[key] = 50
            elif key == 'terrain_comm':
                st.session_state[key] = ""
            else:
                st.session_state[key] = "— Choisir —"
    st.session_state.region_terrain = None
    st.toast("🧹 Formulaire terrain réinitialisé", icon="🧹")
...
.tkr-item .tr{ color:#94a3b8; font-size:12px; }
.tkr-sep{ color:#334155; padding:0 6px; }
...
@keyframes bp{ 0%,100%{box-shadow:0 0 0 0 rgba(22,163,74,0.4)} 50%{box-shadow:0 0 0 8px rgba(22,163,74,0)} }
...
@media (max-width: 768px) {
  ...
  /* On réorganise l'ordre des colonnes : la carte (col_l) passe en premier, puis onglets (col_c), puis droite (col_r) */
  /* On utilise flex-direction: column sur le conteneur principal des colonnes */
  section.main > div {
    display: flex;
    flex-direction: column;
  }
  /* On réordonne les 3 colonnes : l'ordre d'apparition en HTML est col_l, col_c, col_r */
  /* On peut aussi utiliser order, mais plus simple : on définit l'ordre dans le HTML */
  /* Par défaut, col_l, col_c, col_r sont déjà dans cet ordre. Il suffit de les empiler. */
  .stColumn {
    width: 100% !important;
    margin-bottom: 20px;
  }
...
# ═══════════════════════════════════════════════════════════════════════
INFO_MSGS = [
    "🌾 Saison de récolte maïs : prix bas de juin à septembre — stockez si possible",
    "📈 Tomate : prix au pic en décembre-janvier (saison sèche, moins d'offre)",
    "🚛 Douala → Yaoundé : ~3 500 FCFA/sac 50 kg — intégrez ce coût dans votre prix",
    "💡 Groupez vos ventes avec d'autres producteurs pour négocier le transport",
    "☀️ Oignon Grand-Nord : prix bas après récolte nov-jan, hausse dès mars",
    "📊 Nos modèles analysent 94 080 relevés de prix (2019-2024)",
    "🗺️ Littoral (Douala) offre les prix les plus élevés pour les produits transformés",
    "🐟 Tilapia frais : moins cher d'avril à juin (période de pêche intensive)",
    "🌱 Plantain : prix 2× plus bas en saison des pluies (mars-mai) vs saison sèche",
    "📉 Stockage = levier clé — un oignon stocké 6 semaines peut valoir 40% de plus",
]
...
st.markdown(f'<div class="info-bar">📌 {info_msg}</div>', unsafe_allow_html=True)

# ═══════════════════════════════════════════════════════════════════════
# LAYOUT 3 COLONNES (ordres desktop, mobile réorganisé par CSS)
# ═══════════════════════════════════════════════════════════════════════
col_l, col_c, col_r = st.columns([0.95, 1.8, 1.25], gap="medium")

# ═══════════════════════════════════════════════════════════════════════
# COLONNE GAUCHE – CARTE + TENDANCES
# ═══════════════════════════════════════════════════════════════════════
with col_l:
...
# ═══════════════════════════════════════════════════════════════════════
# COLONNE CENTRE – ONGLETS
# ═══════════════════════════════════════════════════════════════════════
with col_c:
    tab_analyse, tab_terrain = st.tabs(["🔍 Analyse prédictive", "📥 Collecte terrain"])
        ...
        with col_btn1:
            if st.button("🔍 Lancer l'Analyse AgriPulse", key="btn_analyse"):
                if produit_choix == "— Choisir —" or region_choix == "— Choisir —" or prix_input <= 0:
                    st.warning("⚠️ Veuillez compléter tous les champs obligatoires (produit, région, prix > 0).")
                else:
        ...
        with col_btn3:
            if st.button("💾 Enregistrer l'observation", key="btn_terrain"):
                ...
                if (f_produit == "— Choisir —" or f_region == "— Choisir —"
                    ...
                    st.warning("⚠️ Veuillez compléter tous les champs obligatoires (produit, région, marché, prix > 50 FCFA).")
                else:
...
# ═══════════════════════════════════════════════════════════════════════
# COLONNE DROITE – Résultats (prédictions, ML, recommandations)
# ═══════════════════════════════════════════════════════════════════════
with col_r:
    if st.session_state.analyse_done and st.session_state.result:
        res = st.session_state.result
    ...
    st.markdown('<div class="h-sec">🤖 Recommandations AgriPulse</div>', unsafe_allow_html=True)
    if st.session_state.analyse_done and st.session_state.reco:
        ...
        to, tl, tt = st.tabs(["💰 Opportunité", "🚛 Logistique", "📐 Technique"])
        with to:
        ...
        with tl:
            ...
            astuces_r = [
                ...
                "🛣️ En saison des pluies, privilégiez les routes bitumées accessibles.",
        ...
        to0, tl0, tt0 = st.tabs(["💰 Opportunité", "🚛 Logistique & Prix", "📐 Notes Techniques"])
        with to0:
            st.markdown("""
    ...
    st.markdown("""
    <div class="ft-bar">
      Propulsé par <strong>AgriPulse</strong> · Yaoundé, Cameroun 🇨🇲<br>
